...
class FoodCustomizatonModal extends React.Component {
  ...
  getCustomizationOptions = async () => {
    const response = await fetch(
      apiURL +
        "food-item-customizations/?food_item_api_key=" +
  ...
  foodOptionSelected = async e => {
    ...
    const optionApiKey = e.target.childNodes[
      e.target.selectedIndex
    ].getAttribute("apikey");
    const optionPrice = e.target.childNodes[
      e.target.selectedIndex
    ].getAttribute("price");
  ...
  verifyAdditionalChargesAreUnique = async selectionApiKeyToCheck => {
    ...
    const verifiedUniqueCustomizations = this.state.additionalCharges.filter(
      charge => {
